(function() {
  ...
  var RPCServer = (function(_super){
    ...
    RPCServer.prototype.connectToRedis = function(){
      ...
      this.redisDB = this.redis.createClient(9891, 'AUTH REDIS');
      this.redisPub = this.redis.createClient(9891, 'AUTH REDIS');

      this.redisDB.auth('AUTH REDIS', function(err){
        if(err){throw err;}
        supra.redisDB.subscribe("broadcast");
        supra.redisDB.on("message", supra.onRedisMessage.bind({_supra:supra}));
      });
      this.redisPub.auth('AUTH REDIS', function(err){
        if(err){throw err;}
      });
      ...
      //Acept a Broadcasting to all connection on the same chanel and domain
        if(data.msg != null && _this.getConnection(conn) != null && data.c != null && conn.chanels[data.c] != null){
            ...
            if(send == true && conn.chanels[data.c] == "write"){
              ...
              var _obj = {pid:_this.pid, _chanel:data.c, _domain:conn._domain, _msg:data.msg};
              _this.redisPub.publish("broadcast", JSON.stringify(_obj));//Broadcast to external connections
            }
            return;
        }
    ...
    RPCServer.prototype.onRedisMessage = function(channel, message){
      ...
      //console.log("Process receiver ID:"+_this.pid+"   Process sender: "+data.pid+"   Message: "+message);
      //console.log("PR:"+_this.pid+" PS:"+data.pid+"  Message: "+message);
      console.log(data);
